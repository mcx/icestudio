...
class WaflePluginExtEmbedded extends WaflePlugin {
  ...
  run() {
    if (this.isRunning()) {
      ...
      this.dom.shadow = iceStudio.gui.addNode(
        this.manifest.id,
        'plugin-wrapper'
      );
      ...
      if (this.manifest.gui.scripts === 'undefined')
        this.manifest.gui.scripts = [];
      if (this.manifest.gui.views === 'undefined') this.manifest.gui.views = [];

      const htmlFiles = _this.manifest.gui.layout.map(
        (file) => `${this.baseUrl()}${file}`
      );
      const styleHostFiles = _this.manifest.gui.styleHost.map(
        (file) => `${this.baseUrl()}${file}`
      );
      assets.getAll(htmlFiles).then((html) => {
        ...
        assets.getAll(styleFiles).then((css) => {
          assets.getAll(styleHostFiles).then((css2) => {
            ...
            const jsfiles = _this.manifest.gui.scripts.map(
              (file) => `${this.baseUrl()}${file}`
            );
            assets.getAll(jsfiles).then((pluginScripts) => {
              const viewfiles = _this.manifest.gui.views.map(
