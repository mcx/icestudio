  ...
  eventExists(eventId, event) {
    if (typeof this.events[eventId] !== 'undefined') {
      for (let e in this.events[eventId]) {
        if (
          event.owner === this.events[eventId].owner &&
    ...
    if (typeof this.events[eventId] !== 'undefined') {
      for (let i = this.events[eventId].length - 1, n = -1; i > n; i--) {
        ...
        if (ownerId === 'all') {
          ...
          typeof this.events[eventId][i].uuid !== 'undefined' &&
          this.events[eventId][i].uuid === ownerId
        ) {
          fire = true;
        }
        if (fire) {
          if (this.events[eventId][i].owner === false) {
            ...
            this.events[eventId][i].owner[this.events[eventId][i].handler](
              eventArgs
            );
          }
        }
